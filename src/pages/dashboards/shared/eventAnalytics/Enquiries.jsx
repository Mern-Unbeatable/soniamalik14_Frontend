...
const mapInquiryToRow = (inquiry) => ({
  ...
  eventListingName: inquiry?.event?.title || inquiry?.service?.listingHeadline || '-',
          ...
          <>
            <div className="space-y-3 p-4 md:hidden">
              {enquiries.map((enquiry, idx) => (
                <div
                  key={`${enquiry.id}-${idx}`}
                  className="rounded-xl border border-[#E7F1F1] bg-[#E7F1F1] p-4"
                >
                  ...
                  <div className="mt-3 space-y-1.5 text-sm text-[#4B5563]">
                    <p>
                      <span className="font-semibold text-[#1D1D1D]">Event/Listing Name:</span>{' '}
                      {enquiry.eventListingName}
                    </p>
                    <p className="break-all">
                      <span className="font-semibold text-[#1D1D1D]">Message:</span>{' '}
                      {enquiry.message}
                    </p>
                    <p>
            ...
            <div className="hidden overflow-x-auto md:block">
              <table className="min-w-245 border-collapse xl:min-w-full">
                <thead>
                  <tr className="border-b border-[#E7F1F1] bg-[#E7F1F1] text-left">
                    ...
                    <th className="px-5 py-3 text-base font-medium text-[#1D1D1D]">
                      Event/Listing Name
                    </th>
                    ...
                    <th className="px-5 py-3 text-center text-base font-medium text-[#1D1D1D]">
                      ACTIONS
                    </th>
                  </tr>
                </thead>
                <tbody>
                  {enquiries.map((enquiry, idx) => (
                    <tr
                      key={`${enquiry.id}-${idx}`}
                      className="border-b border-[#E7F1F1] align-middle last:border-b-0"
                    >
                      <td className="px-5 py-4 text-base font-medium text-[#2F3B3A]">
                        {enquiry.playerName}
                      </td>
                      <td className="px-5 py-4 text-base text-[#4B5563]">{enquiry.phoneNumber}</td>
                      <td className="px-5 py-4 text-base text-[#2F3B3A]">
                        {enquiry.eventListingName}
                      </td>
                      <td className="max-w-90 px-5 py-4 text-base leading-6 text-[#4B5563]">
                        {enquiry.message}
                      </td>
        ...
        >
          ...
          >
            ...
            <div className="max-h-[75vh] overflow-y-auto px-5 pb-6">
              <div className="space-y-2 text-base leading-8 font-semibold text-[#1D1D1D]">

...
const getParticipationLabel = (service) => {
  if (typeof service?.womenOnly === 'boolean') {
    return service.womenOnly ? 'Women-only' : 'Mixed, women welcome';
  }

  const who = String(service?.whoCanTakePart || '')
    .trim()
    .toLowerCase();
  if (!who) return '';
  if (who.includes('mixed') || who.includes('women welcome')) {
    return 'Mixed, women welcome';
  }
  if (who.includes('women-only') || who.includes('women only') || who === 'women_only') {
    return 'Women-only';
  }
...
const mapServiceToDetailsItem = (service) => {
  ...
  return {
    ...
    title:
      ...
      'Untitled Service',
    coach: service?.organizationName || service?.provider?.organizationName || '',
    headCoach: service?.organizationName || service?.provider?.organizationName || '',
    ...
    sportType: Array.isArray(service?.sports)
      ? service.sports.join(', ')
      : service?.sports || 'N/A',
    ...
    womensOnly: getParticipationLabel(service),
    typicalSessionDays:
      scheduleDays ||
      service?.sessonDay ||
...
const RecruitmentDetails = () => {
    ...
    const loadServiceDetails = async () => {
      ...
      try {
        ...
        try {
          ...
          const mappedBookings = (Array.isArray(bookings) ? bookings : []).map(
            (booking, index) => ({
              ...
              phone:
                ...
                booking?.user?.phone ||
                'N/A',
              email: booking?.email || booking?.participantEmail || booking?.user?.email || 'N/A',
            })
          );
        ...
        try {
          ...
          const mappedInterests = (Array.isArray(interests) ? interests : []).map(
            (interest, index) => ({
              ...
              phone:
                ...
                'N/A',
              email:
                interest?.email || interest?.participantEmail || interest?.user?.email || 'N/A',
            })
          );
        ...
        try {
          ...
          const mappedMessages = (Array.isArray(messages) ? messages : []).map(
            (message, index) => ({
              ...message,
              ...
              email:
                ...
                message?.user?.email ||
                'N/A',
              msg: message?.msg || message?.message || message?.content || 'N/A',
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFB] p-4 font-sans text-[#1F2937] md:p-8">
      <div className="space-y-8">
        {/* Back Button */}
        <div>
          <Link
            to={backTarget}
            className="inline-flex items-center text-sm font-semibold text-[#0F766E] hover:underline"
          >
        ...
        {item.bookingLink && (
          <div className="mb-8 rounded-lg border border-gray-100 bg-white p-4 shadow-sm sm:p-6">
            ...
            {item.status === 'PENDING_APPROVAL' || item.status === 'PENDING' ? (
              <p className="mt-1 text-base font-medium text-gray-500 italic">
                You can share after admin approved
              </p>
            ) : (
              <>
                <p className="mb-3 text-base text-gray-500">
                  Use this link to share your ESSA Hub listing on your website, social media or
                  messages.
                </p>
                <div className="flex w-full flex-col items-stretch gap-3 sm:flex-row sm:items-center">
                  <a
                    ...
                    className="block flex-1 rounded-lg border border-gray-200 bg-gray-50 p-3 font-mono text-sm font-medium break-all text-[#0F766E] hover:underline md:text-base"
                  >
                    {item.bookingLink}
                  </a>
                  <button

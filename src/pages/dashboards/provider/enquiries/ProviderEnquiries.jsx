...
const mapInquiryToRow = (inquiry) => ({
  ...
  phone: inquiry?.sender?.phone || inquiry?.sender?.phoneNumber || '-',
    ...
    >
      ...
      >
        ...
        <div className="max-h-[75vh] overflow-y-auto px-5 pb-6">
          ...
          <p className="pt-3 text-base leading-7 text-[#374151]">{enquiry.message}</p>
...
const ProviderEnquiries = () => {
  ...
  const fetchEnquiries = useCallback(async () => {
    try {
      ...
      setEnquiries(inquiries.map(mapInquiryToRow));
      setTotalResults(Number(pagination?.total) || inquiries.length);
  ...
  return (
    <div className="dashboardPy dashboardSpaceY bg-[#F4F6F8]">
      <section className="rounded-lg bg-white">
        ...
        {loading && (
          <div className="px-5 py-10 text-center text-gray-600">Loading enquiries...</div>
        )}

        {!loading && enquiries.length === 0 && (
          <div className="px-5 py-10 text-center text-gray-500">No enquiries found.</div>
        )}
